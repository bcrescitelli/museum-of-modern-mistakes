...
const DrawingCanvas = ({ onSave, prompt, timeLimit }) => {
  ...
  const initCanvas = useCallback(() => {
    ...
    // Only reset if dimensions actually changed (prevents slider-induced clearing)
    ...
    // If we MUST resize, preserve current image
...
export default function App() {
  ...
  // Host Logic logic with race-condition protection
  useEffect(() => {
  ...
  const joinGame = async (code) => {
    ...
    // Generate static prompts for this player on join so they persist
    const myPrompts = [...PROMPTS].sort(() => 0.5 - Math.random()).slice(0, 3);
    const objective = OBJECTIVES[Math.floor(Math.random() * OBJECTIVES.length)];
    
    await setDoc(doc(roomRef, 'players', user.uid), { 
      name, cash: 1000, inventory: [], ready: false, votes: 0, wingTitle: '', objective, assignedPrompts: myPrompts 
    });
  ...
  const handleDrawingSubmit = async (dataUrl) => {
    const me = players.find(p => p.id === user.uid);
    const prompts = me?.assignedPrompts || [];
            ...
            <div className="w-full h-full max-h-[85vh] flex flex-col items-center justify-center animate-in zoom-in duration-700 px-4">
              {players[room.presentingIdx] && (
                <>
                  ...
                  <div className="flex w-full gap-4 justify-center items-start px-12">
                    {items.filter(i => (players[room.presentingIdx].inventory || []).includes(i.id)).map(item => (
                      <div key={item.id} className="flex-1 max-w-[31%] bg-white p-6 rounded-[3.5rem] shadow-2xl relative border-2 border-white transform transition-transform hover:scale-[1.03]">
                        ...
                        <h4 className="text-2xl font-black text-slate-800 leading-tight mb-2 truncate">{item.title}</h4>
        ...
        {!room ? (
          ...
          <>
            {me?.ready ? (
              ...
              <DrawingCanvas key={items.filter(i => i.artistId === user.uid).length} prompt={(me?.assignedPrompts || [])[items.filter(i => i.artistId === user.uid).length] || PROMPTS[0]} timeLimit={90} onSave={handleDrawingSubmit} />
            )}
          </>
