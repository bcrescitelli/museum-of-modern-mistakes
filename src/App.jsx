...
const DrawingCanvas = ({ onSave, prompt, timeLimit }) => {
  ...
  // Robust initialization and resize handling that prevents clearing and angular lines
  const initCanvas = useCallback(() => {
    ...
    context.lineJoin = "round"; // Fix for angular/spikey lines
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // No longer auto-pausing music on Results to keep loop active during restart
  }, [isMuted]);
    ...
    <div className={`min-h-[100dvh] flex flex-col max-w-md mx-auto relative overflow-hidden font-sans transition-colors duration-200 ${isPanic ? 'bg-red-500 animate-pulse' : 'bg-slate-50'}`}>
      ...
      <div className="bg-slate-900 text-white p-4 flex justify-between items-center z-10 shadow-xl border-b border-indigo-500/30 shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-2xl bg-indigo-500 flex items-center justify-center font-black text-3xl shadow-inner shadow-black/30 text-white">
      ...
      <main className="flex-1 overflow-y-auto pb-24">
        {!room ? (
          ...
          <div className="p-12 text-center space-y-8 flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in">
            ...
            <div className={`w-full bg-slate-900 rounded-[4rem] p-12 text-center text-white shadow-2xl relative border-8 ${(me?.inventory?.length || 0) >= 3 ? 'border-red-600' : isPanic ? 'border-white animate-bounce' : 'border-indigo-500'}`}>
              ...
              {(me?.inventory?.length || 0) >= 3 && (
                <div className="absolute inset-0 bg-slate-900/95 flex flex-col items-center justify-center p-8 rounded-[4rem] text-center border-4 border-red-500 animate-in zoom-in">
                  ...
                  <p className="font-black text-3xl text-white italic tracking-tighter leading-none uppercase text-center shrink-0">GALLERY<br/>FULL</p>
                </div>
              )}
            </div>
          ...
          <div className="p-8 space-y-6 animate-in slide-in-from-right">
            {!submittedCuration ? (
              <>
                <div className="bg-indigo-600 text-white p-6 rounded-[2rem] space-y-2 shadow-2xl border-b-[8px] border-indigo-800 shrink-0">
          ...
          <div className="p-12 text-center space-y-10 flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in shrink-0">
             ...
             <p className="text-slate-500 font-bold uppercase tracking-widest text-xs text-slate-900">Witness the modern mistakes on the big screen.</p>
          </div>
        ) : room.phase === PHASES.VOTING && !voted ? (
          <div className="p-8 space-y-8 animate-in slide-in-from-bottom text-slate-900">
            <div className="text-center space-y-1">
              <h2 className="text-5xl font-black uppercase italic tracking-tighter leading-none">Cast Your Vote</h2>
            ...
            <div className="space-y-5">
              {players.filter(p => p.id !== user.uid && p.wingTitle).map(p => (
                ...
                }} className="w-full bg-white p-4 rounded-[2.5rem] border-4 border-slate-200 shadow-xl text-left border-b-[12px] active:translate-y-2 active:border-b-0 transition-all border-indigo-100 flex items-center gap-4 text-slate-900">
          ...
          <div className="p-16 text-center space-y-10 flex flex-col items-center justify-center min-h-[70vh] animate-in fade-in shrink-0">
            ...
            <div className="space-y-4 text-slate-900">
              ...
              <p className="text-slate-500 font-black uppercase tracking-[0.2em] text-sm text-center">Watch the results reveal.</p>
            </div>
          </div>
        )}
      </main>
