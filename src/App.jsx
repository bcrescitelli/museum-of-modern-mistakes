    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-900 p-2 overflow-hidden font-sans select-none touch-none">
      ...
      <div className="flex justify-between items-center p-3 bg-slate-800 rounded-2xl mb-2 shadow-xl shrink-0">
        <div className="flex-1 pr-4">
          <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest leading-none mb-1 text-indigo-400">Subject</p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const claimItem = async () => {
      ...
      if (!currentClaim && available.length > 0) {
        ...
        if (target) {
          // Use transaction to claim item reliably
          try {
    ...
    claimItem();
  }, [room?.phase, items, me?.ready, view, user?.uid, roomId]);
  ...
  useEffect(() => {
    ...
    if (isSettled && players.length > 0 && players.every(p => p.ready)) {
      ...
      else if (room.phase === PHASES.INTRO_VIDEO) { /* Skip handled by end or manual */ }
  ...
  if (view === 'host') {
    return (
      <div className="min-h-screen flex flex-col p-12 overflow-hidden relative bg-slate-100 font-sans text-slate-900 text-slate-900">
          ...
          {room?.phase === PHASES.AUCTION && (
            room.currentAuction ? (
              ...
              <div className="text-center space-y-12 animate-pulse">
                ...
                <h2 className="text-7xl font-black text-slate-800 uppercase italic tracking-[0.2em] text-slate-800">Next Lot...</h2>
              </div>
            )
          )}
                  ...
                  <div className="flex w-full gap-8 justify-center items-start px-12">
                    {items.filter(i => (players[room.presentingIdx].inventory || []).includes(i.id)).sort((a,b) => (players[room.presentingIdx].inventory.indexOf(a.id) - players[room.presentingIdx].inventory.indexOf(b.id))).map(item => (
                      <div key={item.id} className="flex-1 max-w-[31%] bg-white p-6 rounded-[3.5rem] shadow-2xl relative border-2 border-white transform transition-transform hover:scale-[1.03] text-slate-900">
                        ...
                        <h4 className="text-3xl font-black text-slate-800 leading-tight mb-2 truncate text-slate-800 text-slate-800">{item.title}</h4>
                        <p className="text-sm text-slate-500 italic font-bold leading-tight border-t-2 pt-4 border-slate-100 line-clamp-3 text-slate-500 text-slate-500">"{item.history}"</p>
                      </div>
                    ))}
                  </div>
                </>
            ...
            <div className="w-full max-w-5xl space-y-6 animate-in slide-in-from-bottom flex flex-col items-center text-slate-900">
              ...
              <div className="w-full space-y-6 max-h-[60vh] overflow-y-auto px-4">
                {[...players].sort((a,b) => {
                  ...
                  return (
                    <div key={p.id} className="bg-white p-10 rounded-[3rem] shadow-2xl flex items-center justify-between border-l-[30px] border-indigo-500 transform transition-all hover:-translate-x-6 text-slate-900 text-slate-900">
                      <div className="flex items-center gap-12">
                        <span className="text-8xl font-black text-slate-200 text-slate-200">#{i+1}</span>
                        <div>
                      ...
                      <div className="text-9xl font-black text-indigo-600 font-mono tracking-tighter leading-none text-indigo-600">${totalScore}</div>
                    </div>
                  );
                })}
              </div>
        ...
        <button onClick={() => setIsMuted(!isMuted)} className="absolute bottom-12 right-12 bg-white p-8 rounded-full shadow-2xl text-slate-600 z-50 hover:scale-110 transition-transform text-slate-600">
  ...
  return (
    <div className={`min-h-[100dvh] flex flex-col max-w-md mx-auto relative overflow-hidden font-sans transition-colors duration-200 ${isPanic ? 'bg-red-500 animate-pulse' : 'bg-slate-50'}`}>
      ...
      {me?.objective && (
        <div className="bg-indigo-600 text-white px-4 py-2 flex items-start justify-between shadow-lg z-20 border-b border-indigo-700 animate-in slide-in-from-top shrink-0 text-white">
          <div className="flex items-start gap-2">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-[10px] font-bold opacity-80 leading-tight mt-0.5">{me.objective.desc}</p>
            </div>
          </div>
        </div>
      )}

      <div className="bg-slate-900 text-white p-4 flex justify-between items-center z-10 shadow-xl border-b border-indigo-500/30 shrink-0 text-slate-100 text-white">
        <div className="flex items-center gap-3 text-slate-100 text-white text-white">
          <div className="w-14 h-14 rounded-2xl bg-indigo-500 flex items-center justify-center font-black text-3xl shadow-inner shadow-black/30 text-white text-white">{name ? name[0] : me?.name ? me.name[0] : '?'}</div>
          <div>
        ...
        <div className="flex items-center gap-3 bg-slate-800 px-6 py-3 rounded-2xl border border-slate-700 shadow-inner text-slate-100">
          <Coins size={24} className="text-yellow-400" />
          <span className="font-mono font-black text-2xl tracking-tighter leading-none text-white text-white">${me?.cash || 0}</span>
        </div>
      </div>
        ...
        {!room ? (
          ...
          <div className="p-8 text-center space-y-10 flex flex-col items-center justify-center min-h-[60vh]">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-500 font-bold text-lg leading-snug text-slate-500">Exhibition Theme:<br/><span className="text-indigo-600 font-black uppercase italic tracking-tight">{room.theme}</span></p>
            </div>
          </div>
        ) : room.phase === PHASES.INTRO_VIDEO ? (
          <div className="p-12 text-center space-y-8 flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in shrink-0">
             ...
             <h3 className="text-3xl font-black text-slate-900 uppercase italic leading-tight text-slate-900">Rules Briefing</h3>
             <p className="text-slate-500 font-bold uppercase tracking-widest text-xs text-slate-500">Watch the main screen for instructions.</p>
          </div>
          ...
          <>
            {me?.ready ? (
              <div className="p-16 text-center space-y-8 flex flex-col items-center justify-center min-h-[60vh] animate-in slide-in-from-bottom shrink-0">
                ...
                <h3 className="text-4xl font-black text-slate-900 uppercase italic leading-none text-slate-900 text-slate-900">Art Delivered</h3>
                <p className="text-slate-400 font-black tracking-widest uppercase text-xs text-slate-900 text-slate-400">Waiting for curators...</p>
              </div>
            ) : (
              <DrawingCanvas 
          ...
          <div className="p-6 space-y-6 text-slate-900 text-slate-900">
            {me?.ready ? (
               <div className="p-16 text-center space-y-8 flex flex-col items-center justify-center min-h-[60vh] shrink-0">
                 <CheckCircle2 size={140} className="text-indigo-500" />
                 <h3 className="text-4xl font-black text-slate-900 uppercase italic leading-none text-slate-900 text-slate-900">Certified!</h3>
                 <p className="text-slate-400 font-black tracking-widest uppercase text-xs text-slate-400">Auction is opening...</p>
               </div>
            ) : (
              <>
                  ...
                  <div key={item.id} className="space-y-6 animate-in zoom-in">
                    ...
                    <div className="space-y-4 pt-4 text-slate-900">
                      ...
                      <button onClick={() => {
                        ...
                        }} className="w-full py-7 bg-indigo-600 text-white rounded-3xl font-black text-3xl shadow-xl active:scale-95 transition-all border-b-8 border-indigo-800 uppercase tracking-tighter text-white text-white"
                      >Verify Item</button>
                    </div>
                  </div>
                ))}
                {items.filter(i => i.artistId !== user?.uid && !i.appraised && (i.appraiserId === user.uid || !i.appraiserId)).length === 0 && (
                  <div className="p-16 text-center space-y-8 flex flex-col items-center justify-center min-h-[60vh] shrink-0 text-slate-900">
                    <CheckCircle2 size={140} className="text-indigo-500 animate-pulse" />
                    <h3 className="text-2xl font-black text-slate-400 uppercase italic text-slate-400">Waiting for lots...</h3>
                  </div>
                )}
              </>
            )}
          ...
          <div className="p-6 space-y-8 min-h-[85vh] flex flex-col justify-center animate-in fade-in shrink-0 text-slate-900">
            <div className="text-center space-y-1 text-slate-900 text-slate-900">
              <p className={`text-xs font-black uppercase tracking-[0.3em] ${isPanic ? 'text-white' : 'text-slate-400'}`}>Current Lot</p>
            ...
            <div className={`w-full bg-slate-900 rounded-[4rem] p-12 text-center text-white shadow-2xl relative border-8 ${(me?.inventory?.length || 0) >= 3 ? 'border-red-600' : isPanic ? 'border-white animate-bounce' : 'border-indigo-500'}`}>
              ...
              <p className="text-8xl font-black font-mono tracking-tighter leading-none text-white">${room.currentAuction.highestBid}</p>
              ...
              {(me?.inventory?.length || 0) >= 3 && (
                <div className="absolute inset-0 bg-slate-900/95 flex flex-col items-center justify-center p-8 rounded-[4rem] text-center border-4 border-red-500 animate-in zoom-in"><AlertCircle size={80} className="text-red-500 mb-4" /><p className="font-black text-3xl text-white italic tracking-tighter leading-none uppercase text-center shrink-0 text-white">GALLERY<br/>FULL</p></div>
              )}
            </div>
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4 text-white text-white">
                {[10, 25, 50, 75].map(amt => (
                  <button key={amt} disabled={(me?.inventory?.length || 0) >= 3} onClick={() => placeBid((room.currentAuction.highestBid || 0) + amt)} className={`py-6 rounded-3xl font-black text-4xl shadow-xl transition-all border-b-8 active:border-b-0 active:translate-y-2 bg-white text-indigo-600 border-slate-200 text-indigo-600 text-indigo-600`}>+${amt}</button>
                ))}
              </div>
            </div>
          </div>
