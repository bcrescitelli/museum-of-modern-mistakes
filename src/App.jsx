    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-900 p-2 overflow-hidden font-sans select-none touch-none text-white">
      ...
      <div className="py-4 px-2 space-y-4 shrink-0">
        <div className="flex justify-between items-center gap-2">
          <div className="flex gap-2 flex-wrap flex-1 text-white text-white">
...
export default function App() {
  ...
  const distributeAppraisals = async () => {
    ...
    shuffledItems.forEach((item, idx) => {
      let appraiserId = playerIds[idx % playerIds.length];
      if (appraiserId === item.artistId) {
        appraiserId = playerIds[(idx + 1) % playerIds.length];
      }
      batch.update(doc(roomRef, 'items', item.id), { appraiserId });
    });
        ...
        <div className="flex-1 flex items-center justify-center">
          ...
          {room?.phase === PHASES.RESULTS && (
            <div className="w-full max-w-5xl space-y-6 animate-in slide-in-from-bottom flex flex-col items-center text-slate-900">
              <h2 className="text-[8rem] font-black text-center mb-16 flex items-center justify-center gap-10 leading-none text-slate-900 drop-shadow-sm uppercase text-slate-900"><Trophy className="text-yellow-400 text-yellow-400" size={150} /> Results</h2>
        ...
        {!room ? (
          ...
          <div className="p-8 text-center space-y-10 flex flex-col items-center justify-center min-h-[60vh]">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-500 font-bold text-lg leading-snug text-slate-500">Exhibition Theme:<br/><span className="text-indigo-600 font-black uppercase italic tracking-tight text-indigo-600">{room.theme}</span></p>
            </div>
          </div>
        ) : room.phase === PHASES.INTRO_VIDEO ? (
          ...
          <>
            {me?.ready ? (
              <div className="p-16 text-center space-y-8 flex flex-col items-center justify-center min-h-[60vh] animate-in slide-in-from-bottom shrink-0 text-slate-900">
                ...
                <h3 className="text-4xl font-black text-slate-900 uppercase italic leading-none text-slate-900">Art Delivered</h3>
                <p className="text-slate-400 font-black tracking-widest uppercase text-xs text-slate-400">Waiting for curators...</p>
              </div>
            ) : (
              <DrawingCanvas 
          ...
          <div className="p-6 space-y-6 text-slate-900 text-slate-900">
            {me?.ready ? (
               <div className="p-16 text-center space-y-8 flex flex-col items-center justify-center min-h-[60vh] shrink-0 text-slate-900">
                 <CheckCircle2 size={140} className="text-indigo-500 text-indigo-500" />
                 <h3 className="text-4xl font-black text-slate-900 uppercase italic leading-none text-slate-900">Certified!</h3>
                ...
                {items.filter(i => i.artistId !== user?.uid && !i.appraised && i.appraiserId === user.uid).slice(0, 1).map(item => (
                  <div key={item.id} className="space-y-6 animate-in zoom-in text-slate-900">
                    ...
                    <div className="space-y-4 pt-4 text-slate-900">
                      <input type="text" id="appraisal-title" placeholder="Title..." className="w-full p-6 bg-white rounded-2xl border-4 border-slate-200 font-black text-2xl outline-none focus:border-indigo-500 shadow-inner text-slate-900" />
                      <textarea id="appraisal-history" placeholder="Write a short history..." className="w-full p-6 bg-white rounded-2xl border-4 border-slate-200 font-black text-lg outline-none focus:border-indigo-500 h-28 shadow-inner text-slate-900" />
                      <button onClick={() => {
                        ...
                        }} className="w-full py-7 bg-indigo-600 text-white rounded-3xl font-black text-3xl shadow-xl active:scale-95 transition-all border-b-8 border-indigo-800 uppercase tracking-tighter text-white"
                      >Verify Item</button>
                    </div>
                  </div>
                ))}
          ...
          <div className="p-8 space-y-6 animate-in slide-in-from-right text-slate-900 shrink-0">
            {!submittedCuration ? (
              <>
                <div className="bg-indigo-600 text-white p-6 rounded-[2rem] space-y-2 shadow-2xl border-b-[8px] border-indigo-800 shrink-0 text-white"><p className="text-[10px] font-black uppercase tracking-widest opacity-80 leading-none text-white">Theme</p><h2 className="text-2xl font-black leading-tight italic uppercase tracking-tighter text-white">{room.theme}</h2></div>
                <div className="space-y-6 flex-1 text-slate-900">
                  <div className="space-y-2 text-slate-900"><p className="text-xs font-black text-slate-400 uppercase tracking-widest pl-1 leading-none text-slate-400">1. Wing Name</p><input type="text" id="w-title" placeholder="A Clever Title..." className="w-full p-5 bg-white rounded-3xl border-4 border-slate-200 font-black text-xl outline-none focus:border-indigo-500 shadow-inner text-slate-900" /></div>
                  <div className="space-y-3 text-slate-900">
                    <div className="flex justify-between items-end px-1 text-slate-900"><p className="text-xs font-black text-slate-400 uppercase tracking-widest leading-none text-slate-400">2. Set Display Order</p><button onClick={() => setCurationOrder([])} className="text-[10px] font-black text-indigo-500 uppercase underline text-indigo-500">Reset</button></div>
